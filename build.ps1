# ============================================================================
# KORE BUILD SYSTEM v2.1 - Project Ouroboros
# ============================================================================
...
#   .\build.ps1 -ForceRuntimeRebuild # Force rebuild of kore_runtime.o
...
# Compiler paths
$script:BOOTSTRAP_COMPILER = "$BOOTSTRAP_DIR\target\release\korec.exe"

# Latest symlinks (junction points on Windows)
$script:LATEST_BUILD = "$ARTIFACTS_DIR\latest"
$script:LATEST_NATIVE = "$ARTIFACTS_DIR\latest\kore_native.exe"
$script:LATEST_V2 = "$ARTIFACTS_DIR\latest\kore_native_v2.exe"

# Runtime
$script:RUNTIME_SRC = "$RUNTIME_DIR\kore_runtime.c"
$script:RUNTIME_OBJ = "$BUILD_DIR\kore_runtime.o"
$script:RUNTIME_OBJ_ASAN = "$BUILD_DIR\kore_runtime_asan.o"
...
function Clean-Build {
    ...
    $combinedFile = "$BUILD_DIR\korec_build.kr"
    ...
    $legacyArtifacts = @(
        "$ROOT\korec_build.kr",
        "$ROOT\korec_build_v2.kr",
        "$ROOT\kore_native.ll",
        "$ROOT\kore_native_fixed.ll",
        "$ROOT\kore_native_v2.ll",
        "$ROOT\kore_native.exe",
        "$ROOT\kore_native_v2.exe",
        "$ROOT\kore_runtime.o",
        "$ROOT\test.ll",
        "$ROOT\test_codegen.ll",
        "$ROOT\test_korec.ll"
...
function Check-Prerequisites {
    ...
    if (-not (Test-Path $RUNTIME_SRC)) {
        ...
        Write-Err "Please ensure runtime/kore_runtime.c exists"
        exit 1
    }
...
function Combine-Sources {
    ...
    $header = "// ============================================================================`n"
    $header += "// KORE Combined Compiler Source`n"
...
function Compile-WithBootstrap {
    ...
    if (-not (Test-Path $OutputLL)) {
        ...
        $fallbackLL = "$BUILD_DIR\kore_native.ll"
...
function Fix-LLVMIR {
    ...
    foreach ($line in $lines) {
        ...
        if (-not $addedTypes -and $line -match "target triple") {
            $fixed.Add("")
            $fixed.Add("; Additional type definitions for Kore runtime")
...
function Build-Native {
    ...
    Write-Info "BUILDING NATIVE KORE COMPILER"
    ...
    $combinedFile = "$BUILD_DIR\korec_build.kr"
    $llFile = "$BUILD_FOLDER\kore_native.ll"
    $fixedLL = "$BUILD_FOLDER\kore_native_fixed.ll"
    $exeFile = "$BUILD_FOLDER\kore_native.exe"
    ...
    Copy-Item $combinedFile "$BUILD_FOLDER\korec_build.kr" -Force
...
function Build-SelfHosted {
    $separator = "=" * 70
    Write-Info $separator
    Write-Info "BUILDING SELF-HOSTED KORE COMPILER"
    Write-Info "Build ID: $BUILD_TIMESTAMP"
    Write-Info $separator
    ...
    elseif (Test-Path "$BUILD_FOLDER\kore_native.exe") {
        $nativeCompiler = "$BUILD_FOLDER\kore_native.exe"
    }
    else {
    ...
    $combinedFile = "$BUILD_DIR\korec_build.kr"
    $llFile = "$BUILD_FOLDER\kore_native_v2.ll"
    $exeFile = "$BUILD_FOLDER\kore_native_v2.exe"
    ...
    Write-Step "Compiling with native Kore compiler..."
    ...
    $fixedLL = "$BUILD_FOLDER\kore_native_v2_fixed.ll"
    Fix-LLVMIR -InputLL $llFile -OutputLL $fixedLL
...
function Run-Tests {
    ...
    if (Test-Path "$TESTS_DIR\unit") {
        $testFiles += Get-ChildItem "$TESTS_DIR\unit\*.kr" -ErrorAction SilentlyContinue
    }
    if (Test-Path "$TESTS_DIR\integration") {
        $testFiles += Get-ChildItem "$TESTS_DIR\integration\*.kr" -ErrorAction SilentlyContinue
    }
    # Also include root tests
    $testFiles += Get-ChildItem "$TESTS_DIR\*.kr" -ErrorAction SilentlyContinue
...
function Run-SmokeTests {
    ...
    $testFiles = @()
    if (Test-Path $smokeDir1) { $testFiles += Get-ChildItem "$smokeDir1\*.kr" -ErrorAction SilentlyContinue }
    if (Test-Path $smokeDir2) { $testFiles += Get-ChildItem "$smokeDir2\*.kr" -ErrorAction SilentlyContinue }
    $testFiles += Get-ChildItem "$TESTS_DIR\*.kr" -ErrorAction SilentlyContinue
...
function Main {
    ...
    switch ($Target) {
        ...
        "test" { Run-Tests }
        "combine" { 
            Combine-Sources -OutputFile "$BUILD_DIR\korec_build.kr" -Sources $CORE_SOURCES 
        }
        "all" {
            Build-Native
