...
void kore_print_i64(int64_t value) {
    printf("%lld\n", (long long)value);
}

void kore_print_str(const char* str) {
    printf("%s", str);
}

void kore_println_str(const char* str) {
    printf("%s\n", str);
}
...
int64_t kore_str_len(const char* str) {
    return (int64_t)strlen(str);
}

int64_t kore_str_eq(const char* a, const char* b) {
...
int64_t kore_array_new() {
    KoreArray* arr = (KoreArray*)malloc(sizeof(KoreArray));
    arr->data = NULL;
    arr->len = 0;
    arr->cap = 0;
    return (int64_t)arr;
...
int64_t kore_contains(int64_t arr_ptr, int64_t item_ptr) {
    KoreArray* arr = (KoreArray*)arr_ptr;
...
int64_t kore_split(int64_t str_ptr, int64_t delim_ptr) {
    const char* str = (const char*)str_ptr;
    const char* delim = (const char*)delim_ptr;
    
    int64_t arr_ptr = kore_array_new();
    ...
    if (strlen(delim) == 0) {
        ...
        for (size_t i = 0; i < len; i++) {
            ...
            buf[0] = str[i];
            buf[1] = '\0';
            kore_array_push(arr_ptr, (int64_t)kore_str_new(buf));
        }
    } else {
        // Split by delimiter
        ...
        while (token != NULL) {
            kore_array_push(arr_ptr, (int64_t)kore_str_new(token));
...
int64_t kore_len(int64_t obj_ptr) {
    // Hack: try to guess if it's an array or string
    // In lexer.kr, len() is used for both.
    // Since we can't easily distinguish without tags, and lexer usage involves both:
    // len(tokens) -> Array
    // len(chars) -> Array
    // len(lexeme) -> String
    
    // If we assume pointers are valid...
    // KoreArray is 24 bytes.
    // String is variable.
    
    // Let's try to interpret as KoreArray first?
    // Arrays have high capacity/len usually small?
    // This is dangerous.
    
    // For now, defaulting to Array len because `contains` implies array operations are active.
    // Strings might break.
    // TODO: Implement tagged pointers or object headers.
    return kore_array_len(obj_ptr);
}

int64_t kore_to_int(int64_t str_ptr) {
    const char* str = (const char*)str_ptr;
    return atoll(str);
}
...
int64_t kore_to_float(int64_t str_ptr) {
...
int64_t kore_to_string(int64_t val) {
    ...
    return (int64_t)kore_str_new(buf);
}

int64_t kore_range(int64_t start, int64_t end) {
    int64_t arr_ptr = kore_array_new();
    for (int64_t i = start; i < end; i++) {
        kore_array_push(arr_ptr, i);
    }
    return arr_ptr;
}

int64_t kore_char_at(int64_t str_ptr, int64_t index) {
    const char* str = (const char*)str_ptr;
    if (index < 0 || index >= (int64_t)strlen(str)) return 0;
    char buf[2];
    buf[0] = str[index];
    buf[1] = '\0';
    return (int64_t)kore_str_new(buf);
...
int64_t kore_substring(int64_t str_ptr, int64_t start, int64_t end) {
    const char* str = (const char*)str_ptr;
    size_t len = strlen(str);
    if (start < 0) start = 0;
    if (end > (int64_t)len) end = len;
    if (start >= end) return (int64_t)kore_str_new("");
...
int64_t kore_slice(int64_t arr_ptr, int64_t start, int64_t end) {
    KoreArray* arr = (KoreArray*)arr_ptr;
...
int64_t kore_append(int64_t str_ptr1, int64_t str_ptr2) {
    const char* a = (const char*)str_ptr1;
    const char* b = (const char*)str_ptr2;
    return (int64_t)kore_str_concat(a, b);
}
...
int64_t kore_some(int64_t value) {
    KoreOption* opt = (KoreOption*)malloc(sizeof(KoreOption));
    opt->tag = 1;
    opt->value = value;
    return (int64_t)opt;
}
...
int64_t kore_unwrap(int64_t opt_ptr) {
    KoreOption* opt = (KoreOption*)opt_ptr;
    if (opt->tag == 0) {
...
#define KORE_TAG_INT    0
#define KORE_TAG_FLOAT  1
#define KORE_TAG_STRING 2
#define KORE_TAG_BOOL   3
#define KORE_TAG_ARRAY  4
#define KORE_TAG_NONE   5
...
int64_t kore_contains_key(int64_t map_ptr, int64_t key_ptr) {
    KoreMap* map = (KoreMap*)map_ptr;
...
int64_t kore_map_get(int64_t map_ptr, int64_t key_ptr) {
    KoreMap* map = (KoreMap*)map_ptr;
    const char* key = (const char*)key_ptr;
    for (int64_t i = 0; i < map->len; i++) {
        const char* stored_key = (const char*)map->keys[i];
...
// Map insert/update
void kore_map_set(int64_t map_ptr, int64_t key_ptr, int64_t value) {
    KoreMap* map = (KoreMap*)map_ptr;
    const char* key = (const char*)key_ptr;
...
int64_t kore_join(int64_t arr_ptr, int64_t delim_ptr) {
    KoreArray* arr = (KoreArray*)arr_ptr;
    const char* delim = (const char*)delim_ptr;
    
    if (arr->len == 0) {
        return (int64_t)kore_str_new("");
    }
...
int64_t kore_variant_of(int64_t value_ptr) {
    ...
    if (!ptr) return (int64_t)kore_str_new("None");
    int64_t tag = *ptr;
    char buf[32];
...
int64_t kore_variant_field(int64_t value_ptr, int64_t field_idx) {
