...
impl WasmCompiler {
    ...
    fn compile_program(&mut self, program: &TypedProgram) -> KoreResult<()> {
    ...
    fn compile_component(&mut self, c: &crate::types::TypedComponent) -> KoreResult<()> {
    ...
    fn compile_lambda(&mut self, id: u32, params: &[crate::ast::Param], body: &Expr) -> KoreResult<()> {
    ...
    fn declare_function(&mut self, func: &TypedFunction) -> KoreResult<()> {
        let (param_types, ret_type) = if let ResolvedType::Function { params, ret, .. } = &func.resolved_type {
            (params, ret)
        } else {
            return Err(KoreError::codegen("Expected function type", func.ast.span));
        };
    ...
    fn compile_function_body(&mut self, func: &TypedFunction) -> KoreResult<()> {
        ...
        let (_dummy_global_id, dummy_import_id) = self.module.add_import_global("KORE_internal", "dummy", ValType::I32, false, false);
    ...
    fn compile_block(&self, ctx: &CompilationContext, builder: &mut InstrSeqBuilder, block: &Block) -> KoreResult<()> {
        for stmt in &block.stmts {
    ...
    fn compile_stmt(&self, ctx: &CompilationContext, builder: &mut InstrSeqBuilder, stmt: &Stmt) -> KoreResult<()> {
        match stmt {
            Stmt::Expr(expr) => {
    ...
    fn compile_expr(&self, ctx: &CompilationContext, builder: &mut InstrSeqBuilder, expr: &Expr) -> KoreResult<()> {
        match expr {
            Expr::Int(n, _) => {
                builder.i64_const(*n);
            }
            ...
            Expr::String(s, span) => {
                ...
                if let Some(&offset) = ctx.string_table.get(s) {
                    ...
                    return Err(KoreError::codegen("String not found in table", *span));
                }
            }
            ...
            Expr::Ident(name, span) => {
                if let Some(local_id) = ctx.locals.get(name) {
                    builder.local_get(*local_id);
                     ...
                     return Err(KoreError::codegen(format!("Variable '{}' not found in locals", name), *span));
                }
            }
            Expr::If { condition, then_branch, else_branch, .. } => {
            ...
            Expr::Call { callee, args, span } => {
                ...
                if let Expr::Ident(func_name, _) = callee.as_ref() {
                    ...
                    if let Some(func_id) = ctx.functions.get(func_name) {
                        ...
                        return Err(KoreError::codegen(format!("Function '{}' not found", func_name), *span));
                    }
                } else {
                    // For now, only support direct function calls by name
                    return Err(KoreError::codegen("Only direct function calls supported in WASM", *span));
                }
            }
            ...
            Expr::EnumVariant { enum_name, variant, fields, span } => {
                if let Some((tags, max_payload, field_offsets_map)) = ctx.enum_layouts.get(enum_name) {
                     let tag = *tags.get(variant).ok_or_else(|| KoreError::codegen("Variant tag not found", *span))?;
                    ...
                    return Err(KoreError::codegen(format!("Enum layout not found for {}", enum_name), *span));
                }
            }
            Expr::Struct { name, fields, span } => {
                if let Some((field_offsets, total_size)) = ctx.struct_layouts.get(name).cloned() {
                    ...
                    return Err(KoreError::codegen(format!("Struct '{}' layout not found", name), *span));
                }
            }
            ...
            Expr::MethodCall { receiver, method, args, span } => {
                ...
                if let Some(func_id) = ctx.functions.get(method) {
                    ...
                    return Err(KoreError::codegen(format!("Method '{}' not found", method), *span));
                }
            }
    ...
    fn compile_else_branch(&self, ctx: &CompilationContext, builder: &mut InstrSeqBuilder, branch: &crate::ast::ElseBranch) -> KoreResult<()> {
        match branch {
    ...
    fn compile_jsx_node(&self, ctx: &CompilationContext, builder: &mut InstrSeqBuilder, node: &crate::ast::JSXNode) -> KoreResult<()> {
        match node {
            crate::ast::JSXNode::Element { tag, attributes, children, .. } => {
                // 1. Compile Children
                for child in children {
