...
function add(...args) {
    if (!__wasmReady) {
        console.warn('[KAIN] WASM not ready, add call failed');
        return 0;
    }
    try {
        const result = __wasmInstance.exports.add(...args);
        return result;
    } catch (e) {
        console.error('[KAIN] Error calling add:', e);
        throw new Error(`WASM function 'add' failed: ${e.message}`);
    }
}

async function add_async(...args) {
    await __ensureWasm();
    return add(...args);
}
