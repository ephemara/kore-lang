# KAIN Shader Examples

This folder contains production-quality GPU shader examples written in KAIN, demonstrating the language's expressiveness for graphics programming.

## Shaders

### `pbr_material.kn` - Physically-Based Rendering
...
### `volumetric_clouds.kn` - Raymarched Volumetrics
...
### `sdf_raymarcher.kn` - Signed Distance Fields
...
### `post_processing.kn` - Post-FX Stack
...
These shaders target the KAIN V1 compiler's SPIR-V backend:

```bash
# Compile to SPIR-V
cd kain-v1-stable
cargo build --release
./target/release/kain ../shaders/pbr_material.kn --target spirv -o ../shaders/pbr_material.spv
...
KAIN shader syntax is designed to be familiar to GLSL/HLSL users while being more concise:
...
5. **Composability** - Functions work like any other KAIN code
