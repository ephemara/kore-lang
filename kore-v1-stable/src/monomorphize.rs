...
pub fn monomorphize(program: &TypedProgram) -> KoreResult<MonomorphizedProgram> {
...
impl MonoContext {
    ...
    fn instantiate(&mut self, name: &str, type_args: &[ResolvedType]) -> KoreResult<String> {
        ...
        let generic_func = self.generic_functions.get(name)
            .ok_or_else(|| KoreError::type_error(format!("Generic function {} not found", name), crate::span::Span::new(0,0)))?
            .clone();
            
        if generic_func.ast.generics.len() != type_args.len() {
             return Err(KoreError::type_error(format!("Generic arg count mismatch for {}: expected {}, got {}", name, generic_func.ast.generics.len(), type_args.len()), generic_func.ast.span));
        }
...
fn resolve_ast_type(ty: &Type) -> KoreResult<ResolvedType> {
...
fn lower_async_fn(ctx: &mut MonoContext, func: &TypedFunction) -> KoreResult<TypedFunction> {
...
fn collect_awaits_from_stmt(stmt: &Stmt, points: &mut Vec<AwaitPoint>) {
    match stmt {
        ...
        // Note: In KORE, if is an expression, not a statement. If used in Stmt::Expr, 
...
fn wrap_stmt_returns(stmt: &mut Stmt, span: crate::span::Span) {
    match stmt {
        ...
        // Note: In KORE, if is an expression. Expr::If would be in Stmt::Expr, 
...
fn scan_function(ctx: &mut MonoContext, func: &TypedFunction) -> KoreResult<TypedFunction> {
...
fn scan_block(ctx: &mut MonoContext, env: &mut MonoTypeEnv, block: &mut Block) -> KoreResult<()> {
    env.push();
    for stmt in &mut block.stmts {
...
fn scan_stmt(ctx: &mut MonoContext, env: &mut MonoTypeEnv, stmt: &mut Stmt) -> KoreResult<()> {
    match stmt {
...
fn scan_expr(ctx: &mut MonoContext, env: &mut MonoTypeEnv, expr: &mut Expr) -> KoreResult<ResolvedType> {
    match expr {
