//! Pretty error reporting for KORE
...
impl<'a> Diagnostics<'a> {
    ...
    pub fn format_error(&self, error: &KoreError) -> String {
        match error {
            KoreError::Lexer { message, span } => self.format_with_context("Lexer Error", message, *span),
            KoreError::Parser { message, span } => self.format_with_context("Parse Error", message, *span),
            KoreError::Type { message, span } => self.format_with_context("Type Error", message, *span),
            KoreError::Effect { message, span } => self.format_with_context("Effect Error", message, *span),
            KoreError::Borrow { message, span } => self.format_with_context("Borrow Error", message, *span),
            KoreError::Codegen { message, span } => self.format_with_context("Codegen Error", message, *span),
            KoreError::Runtime { message } => format!(
                "\n\x1b[1;31merror\x1b[0m: {}\n",
                message
            ),
            KoreError::Io(e) => format!(
...
pub fn format_simple_error(error: &KoreError) -> String {
    match error {
        KoreError::Runtime { message } => format!("Runtime Error: {}", message),
        _ => format!("{}", error),
    }
}
...
mod tests {
    ...
    fn test_line_info() {
        let source = "let x = 5\nlet y = x + 1\nprint(y)";
        let diag = Diagnostics::new(source, "test.kr");
