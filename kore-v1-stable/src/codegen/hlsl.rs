//! HLSL Code Generation - Direct KORE to HLSL/USF
...
pub fn generate(program: &TypedProgram) -> KoreResult<String> {
    ...
    // HLSL header
    output.push_str("// Generated by KORE Compiler\n");
...
fn emit_block(ctx: &mut HLSLContext, block: &Block) -> KoreResult<String> {
    let mut output = String::new();
...
fn emit_stmt(ctx: &mut HLSLContext, stmt: &Stmt) -> KoreResult<String> {
    let mut output = String::new();
...
fn emit_expr(ctx: &mut HLSLContext, expr: &Expr) -> KoreResult<(String, String)> {
    match expr {
        Expr::Ident(name, _) => {
        ...
        Expr::Binary { left, op, right, .. } => {
            ...
            let op_str = match op {
                ...
                BinaryOp::Shl => "<<",
                BinaryOp::Shr => ">>",
                _ => return Err(KoreError::codegen("Unsupported binary op", expr.span())),
            };
        ...
        Expr::Call { callee, args, .. } => {
            if let Expr::Ident(name, _) = &**callee {
                ...
                Err(KoreError::codegen("Complex callee not supported", expr.span()))
            }
        },
        ...
        Expr::If { condition, then_branch, else_branch, .. } => {
            ...
            Err(KoreError::codegen("Complex if expressions not yet supported in HLSL backend", expr.span()))
        },
        Expr::Paren(inner, _) => {
        ...
        _ => Err(KoreError::codegen("Unsupported expression", expr.span())),
    }
}

fn emit_function_call(ctx: &mut HLSLContext, name: &str, args: &[crate::ast::CallArg]) -> KoreResult<(String, String)> {
    match name {
        // Vector constructors
        "vec2" | "Vec2" => {
        ...
        "flat" | "noperspective" | "centroid" => {
            ...
            Err(KoreError::codegen(format!("{} is an interpolation modifier, not a function", name), crate::span::Span::new(0, 0)))
        },
        ...
        _ => Err(KoreError::codegen(format!("Unknown function: {}", name), crate::span::Span::new(0, 0))),
    }
}
