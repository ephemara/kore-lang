...
fn diagnostic_from_error(text: &str, err: &KoreError) -> Vec<Diagnostic> {
    let (message, span) = match err {
        KoreError::Lexer { message, span } => (message.clone(), *span),
        KoreError::Parser { message, span } => (message.clone(), *span),
        KoreError::Type { message, span } => (message.clone(), *span),
        KoreError::Effect { message, span } => (message.clone(), *span),
        KoreError::Borrow { message, span } => (message.clone(), *span),
        KoreError::Codegen { message, span } => (message.clone(), *span),
        KoreError::Runtime { message } => (message.clone(), Span::default()),
        KoreError::Io(_) => return vec![],
    };
...
impl LanguageServer for Backend {
    ...
    async fn initialized(&self, _: InitializedParams) {
        self.client
            .log_message(MessageType::INFO, "KORE Language Server initialized!")
            .await;
    }
