...
use crate::error::KoreResult;
use crate::span::Span;

pub fn eval_program(program: &mut Program) -> KoreResult<()> {
    let mut env = Env::new();
...
fn eval_item(env: &mut Env, item: &mut Item) -> KoreResult<()> {
    match item {
...
fn eval_block(env: &mut Env, block: &mut Block) -> KoreResult<()> {
...
fn eval_stmt(env: &mut Env, stmt: &mut Stmt) -> KoreResult<()> {
    match stmt {
...
fn eval_expr_in_place(env: &mut Env, expr: &mut Expr) -> KoreResult<()> {
...
fn eval_jsx(env: &mut Env, node: &mut JSXNode) -> KoreResult<()> {
    match node {
