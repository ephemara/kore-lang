...
use kore::{compile, CompileTarget, VERSION, LANGUAGE_NAME};
use kore::packager;
use kore::lsp;
    ...
    match compile(&source, target) {
        Ok(compiled_output) => {
            if target == CompileTarget::Interpret || target == CompileTarget::Test {
                ...
                if target == CompileTarget::Llvm {
                    ...
                    let runtime_c = std::path::Path::new("src/runtime/c/kore_runtime.c");
                    if runtime_c.exists() {
                         let runtime_o = if cfg!(windows) { "src/runtime/c/kore_runtime.obj" } else { "src/runtime/c/kore_runtime.o" };
        ...
        Err(e) => {
            ...
            let filename = input.file_name()
                .and_then(|s| s.to_str())
                .unwrap_or("input.kr");
            let diag = kore::diagnostics::Diagnostics::new(&source, filename);
...
fn run_ue5_shader_pipeline(input: &PathBuf, args: &Args) -> bool {
    ...
    let compiled_spv = match compile(&source, CompileTarget::SpirV) {
        Ok(bytes) => bytes,
        Err(e) => {
            let filename = input.file_name().and_then(|s| s.to_str()).unwrap_or("input.kr");
            let diag = kore::diagnostics::Diagnostics::new(&source, filename);
            eprint!("{}", diag.format_error(&e));
            return false;
        }
