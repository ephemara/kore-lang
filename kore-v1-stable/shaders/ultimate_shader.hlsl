...
Texture2D albedo_map : register(t5);
SamplerState albedo_map_sampler : register(s5);
Texture2D normal_map : register(t6);
SamplerState normal_map_sampler : register(s6);
Texture2D roughness_map : register(t7);
SamplerState roughness_map_sampler : register(s7);
Texture2D metallic_map : register(t8);
SamplerState metallic_map_sampler : register(s8);
Texture2D ao_map : register(t9);
SamplerState ao_map_sampler : register(s9);
Texture2D emissive_map : register(t10);
SamplerState emissive_map_sampler : register(s10);
Texture2D height_map : register(t11);
SamplerState height_map_sampler : register(s11);
Texture2D detail_map : register(t12);
SamplerState detail_map_sampler : register(s12);
...
PSOutput PSMain(VSInput input)
{
    ...
    float height = height_map.Sample(height_map_sampler, input.uv).r;
    float2 parallax_offset = (view_dir.xy * ((height * 0.100000)));
    float4 parallax_uv = (input.uv - parallax_offset);
    float3 albedo = albedo_map.Sample(albedo_map_sampler, parallax_uv).rgb;
    float3 normal_sample = normal_map.Sample(normal_map_sampler, parallax_uv).rgb;
    float roughness_sample = roughness_map.Sample(roughness_map_sampler, parallax_uv).r;
    float metallic_sample = metallic_map.Sample(metallic_map_sampler, parallax_uv).r;
    float ao_sample = ao_map.Sample(ao_map_sampler, parallax_uv).r;
    float3 emissive = emissive_map.Sample(emissive_map_sampler, parallax_uv).rgb;
    float3 detail = detail_map.Sample(detail_map_sampler, (parallax_uv * 4.000000)).rgb;
    ...
    float r_channel = albedo_map.Sample(albedo_map_sampler, (distorted_uv + r_offset)).r;
    float g_channel = gamma_corrected.g;
    float b_channel = albedo_map.Sample(albedo_map_sampler, (distorted_uv + b_offset)).b;
